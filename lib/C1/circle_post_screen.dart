...
class _CirclePostScreenState extends State<CirclePostScreen> {
  ...
  // 選択項目の初期値
  String _selectedCampus = '豊洲';   // 豊洲, 大宮, 両方
  String _selectedCategory = '運動系'; // 運動系, 文化系, その他

  // 💡 設計書・仕様書テスト用：画像アップロードのシミュレーション変数
  bool _isImageSelected = false;
  bool _simulateLargeFile = false; // 「1MB超えのエラー」を発生させるためのデバッグスイッチ
  ...
  // 💡 投稿ボタンが押された時のバリデーション＆処理
  void _submitPost() {
    final name = _nameController.text.trim();
    final desc = _descController.text.trim();

    // ① 設計書要件：必須項目が未入力の場合のエラー処理
    if (name.isEmpty || desc.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('必須項目が入力されていません'), // 設計書指定の文言
    ...
    // ② 仕様書要件：ファイルサイズが1MBを超えている場合のエラー処理
    if (_isImageSelected && _simulateLargeFile) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('ファイルサイズが大きすぎます。1MB以下のファイルを選択してください。'), // 設計書指定の文言
          backgroundColor: Colors.red,
        ),
      );
      return;
        ...
        child: Column(
          ...
          children: [
            ...
            InkWell(
              onTap: () {
                setState(() {
                  _isImageSelected = !_isImageSelected; // タップで選択/未選択切り替え
                });
              },
              child: Container(
